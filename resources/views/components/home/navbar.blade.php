...
<div class="w-full justify-between navbar bg-base-100 mt-2 drop-shadow-lg flex sticky top-0 z-10">
    <a href="{{ url()->route('home') }}" class=" btn-circle w-16 me-2 lg:me-0 lg:-mt-4 lg:ms-10">
        <img alt="YTFilt" src="{{ asset('logo/ytfilt.png') }}" />
    </a>
    <label
        class="w-full lg:w-[50%] input input-bordered relative items-center pe-0 rounded-full hover:scale-110 transition ease-in-out hover:shadow-lg border-none bg-zinc-100 lg:-ms-24"
        id="searchBar">
        <input type="text" placeholder="Search" wire:model="search" type="search" wire:keydown.enter="searchDirect"
            class="border-3 border-gray-600 w-[90%] h-11" />
        <button wire:click="searchDirect" class="btn bg-[#D90B17] rounded-e-full max-w-min absolute right-0">
...
<div class="navbar mt-2 flex flex-col sticky top-10 z-10">
    <div class="max-w-full lg:w-[50%] flex flex-row justify-between">
        <button id="leftArrow" class="bg-white border shadow-md text-slate-500 hover:scale-105 p-2 rounded-full">
            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg">
